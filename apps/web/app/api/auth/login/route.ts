...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user) return Response.json({ error: 'Credenziali non valide' }, { status: 401 })

    const ok = await verifyPassword(password, user.password_hash)
    if (!ok) return Response.json({ error: 'Credenziali non valide' }, { status: 401 })

    const token = signJWT({ uid: user.id, email: user.email })
    setSessionCookie(token)

    return Response.json({ ok: true, uid: user.id, email: user.email, nickname: user.nickname })
  } catch (err:any) {
